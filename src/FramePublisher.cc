...
FramePublisher::FramePublisher(Map* pMap):mpMap(pMap)
{
    ...
    std::cout<<"published frame done"<<std::endl;
...
void FramePublisher::PublishFrame()
{
    ...
    rosImage.image = im.clone();
        std::cout<<"PublishFrame 3"<<std::endl;

    rosImage.header.stamp = mNH->now();
    rosImage.encoding = "bgr8";
    std::cout<<"PublishFrame 4"<<std::endl;

    mImagePub->publish(rosImage.toImageMsg());

        std::cout<<"PublishFrame 5"<<std::endl;
